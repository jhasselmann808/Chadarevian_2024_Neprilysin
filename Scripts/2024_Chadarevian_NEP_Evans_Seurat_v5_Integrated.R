...
Tcols <- c("forestgreen", "#d89000", "#00b0f6", "darkred", "firebrick1", "darkorchid4")
...
samps <- c("Control", "Met1", "Met2")
ID <- "Control_Met1and2_Combined"
...
# Remove extra dividing cells
Div <- read.table("/path/to/AdditionalDividingCells.tsv", header = T)
scRNA <- subset(scRNA, cells = Div$Cells, invert = T)
...
metaName <- "Condition"
scRNA$Condition <- factor(scRNA$Condition, levels = c("Control", "Metastasis"))

# Create a list of new cluster IDs to be added to the object
new.cluster.ids <- c("Homeostatic", "MHCII", "IFN", "Macros", "IL1B")
...
# Manually assign TAM identity and update object identities
TAM <- read.table("/path/to/TAMCells.tsv",
                  header=T)
scRNA <- SetIdent(scRNA, cells = TAM$Cells, value = "TAM")
scRNA$Clusters <- Idents(scRNA)
scRNA$Clusters <- factor(scRNA$Clusters, levels = c("Homeostatic", "MHCII", "IFN", "TAM", 
                                                    "Macros", "IL1B"))
...
top10$cluster <- factor(top10$cluster, levels = c("Homeostatic", "MHCII", "IFN", "TAM", 
                                                  "Macros", "IL1B"))
...
ID1 <- c("TAM")
ID2 <- c("Homeostatic", "MHCII", "IFN", "Macros", "IL1B")
...
geneList <- c("IFIT1", "IFI6", "IFITM1", "CXCL10", "ISG20", "ABCG2", "MRC1", "LYVE1", "SSBP2", "MAF")
