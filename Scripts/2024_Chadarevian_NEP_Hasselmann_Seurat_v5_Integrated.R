...
set.seed(030989)
# Setting the color palettes for the output files. Tcols currently has colors for up to 15 clusters.
Tcols <- c("forestgreen", "#d89000", "#00b0f6", "darkred", "gray40", "firebrick1")
  ...
  if (TRUE) {
    ...
    volc <- ggplot(input, aes(avg_log2FC, -log10(p_val_adj))) + #volcano plot with log2Foldchange versus pvalue
      ...
      geom_label_repel(data=finLabs, color="white", size=8, segment.color = 'black',
                       force = 400, aes(label=gene, fill = alpha(c(Color), 0.7))) +
...
# =========================================================================================================
# ==================================== Read in previous Seurat objects ====================================
# =========================================================================================================
...
samps <- c("WT_Female", "5X_Female", "WT_Male", "5X_Male")
ID <- "WT_5X_BothSexes_Combined"

# Create variables for the day and time of the analysis
...
outDir <- file.path(storageDir, study, Exp, "Seurat", "Integrated", paste0(day, "_Analysis"), ID)
...
for (i in 1:length(samps)) {
  print(paste0("Loading dataset ", samps[i], "..."))
  assign(paste0("scRNA_", as.character(samps[i])), 
         ReadSample(samps[i]))
}
objList <- lapply(ls(pattern = "scRNA_*"), get)
scRNA <- merge(objList[[1]], c(objList[2:length(objList)]))

# Saving the Seurat object
saveRDS(scRNA, file = file.path(outDir, paste0(Exp, "_", ID, "_Seurat_Object_PreIntegration.rds")))
...
# Normalize the individual datasets
scRNA <- NormalizeData(scRNA)

# Find variable features for each dataset
scRNA <- FindVariableFeatures(scRNA, assay = "RNA", nfeatures = 3000)

# Read in technical gene list from Neuron paper
Tech <- read.table("/path/to/Hasselmann_TechnicalGenes.tsv", header = T)
...
PCAFeats <- subset(PCAFeats, !(PCAFeats$Gene %in% Tech$Gene))
...
# Generate and save an elbow plot
...
# Set factor levels for "Genotype" variable
scRNA$Genotype <- factor(scRNA$Genotype, levels = c(unique(scRNA$Genotype)[[2]], 
                                                  unique(scRNA$Genotype)[[1]]))

# Create a list of new cluster IDs to be added to the object
new.cluster.ids <- c("Homeostatic", "MHCII", "IFN", "DAM", "Secretory", "Macros")
...
ID1 <- c("DAM")
ID2 <- c("Homeostatic", "MHCII", "IFN", "Secretory", "Macros")
...
geneList <- c("CD9", "CCL3", "GPNMB", "LGALS3", "OLR1", "P2RY12", "P2RY13", "CST3", "VSIR", "LYVE1")
...
genes <- c("P2RY12", "P2RY13", "CX3CR1", "SELPLG", "CST3", "OLFML3", "VSIR", "SPI1", "TMEM119",
           ...
           "BIN1", "CD33", "MKI67", "XRCC5", "IFITM3", "TFE3", "TFEC", "TFEB", "MITF", "EGR2", "ATF3",
           "PLAT", "CCR7", "PPARD", "ACY3", "IL4I1", "LY6E", "IFIT3", "XAF1")
